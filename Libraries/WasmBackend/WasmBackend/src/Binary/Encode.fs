...
module Encode =
    ...
    let rec GenInstruction = function
        ...
        | Load(Memory mem, _) when mem.ty = I32() -> &0x28 => GenMemArg mem
        | Load(Memory mem, _) when mem.ty = I64() -> &0x29 => GenMemArg mem
        | Load(Memory mem, _) when mem.ty = F32() -> &0x2A => GenMemArg mem
        | Load(Memory mem, _) when mem.ty = F64() -> &0x2B => GenMemArg mem 
        ...
        | Store(Memory (mem: memop<_,_>), _) when mem.ty = I32() -> &0x36 => GenMemArg mem
        | Store(Memory (mem: memop<_,_>), _) when mem.ty = I64() -> &0x37 => GenMemArg mem
    ...
    let GenElement = function
        ...
        | { ty = refType.FuncRef; init = y; mode = elemmode.Passive} when OnlyIdx y -> 
            &0x01 => &0x00 => elmkind => GenVec Idx (GetFuncIdx y)

        | { ty = refType.FuncRef; init = y ; mode = elemmode.Active frame } when OnlyIdx y ->
            &0x02 => Idx frame.index => GenExpr frame.offset => &0x00 => elmkind => GenVec Idx (GetFuncIdx y)

        | { ty = refType.FuncRef; init = y ; mode = elemmode.Declarative } when OnlyIdx y ->
            &0x03 => &0x00 => elmkind => GenVec Idx (GetFuncIdx y)
    ...
    let GenData : data<'id, 'fid, 'mem, 'info> -> byte seq = function 
        | { init = b; mode = Active mode } when mode.index = 0 -> &0x00 => GenExpr mode.offset => Idx b.Length => b
        | { init = b; mode = Passive } -> &0x01 => Idx b.Length => b
        | { init = b; mode = Active mode } -> &0x02 => Idx mode.index => GenExpr mode.offset => Idx b.Length => b
