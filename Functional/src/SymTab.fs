...
type SymbolTable<'id,'item when 'id: equality and 'id: comparison> = SymTable of ('id * 'item) list with
    static member Empty() = SymTable []
    ...
    interface ISymbolic<'id,'item> with
        member tab.Empty() = SymbolTable<'id,'item>.Empty() :> _
...
type SymbolMap<'id,'item when 'id: equality and 'id: comparison> = SymMap of Map<'id,'item> with
    static member Empty() = SymMap (map[])
    member tab.Bind name item = 
        let (SymMap tab) = tab
    ...
    interface ISymbolic<'id,'item> with
        member tab.Empty() = SymbolMap<'id,'item>.Empty() :> _
        member tab.Bind name item = tab.Bind name item :> _
        member tab.Delete name = tab.Delete name :> _
        member tab.LookUp name = tab.LookUp name 
...
with    
    static member Empty() = Table(0, SymTable []) 

    member tab.Bind name item =
        let (Table(count, tab)) = tab
        if count = 150 then
            let (SymTable tab) = tab :?> SymbolTable<_,_>
    ...
    interface ISymbolic<'id,'item> with
        member tab.Empty() = DynamicTable<'id,'item>.Empty() :> _
        member tab.Bind name item = tab.Bind name item :> _
